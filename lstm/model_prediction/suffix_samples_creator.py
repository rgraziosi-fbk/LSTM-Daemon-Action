...
class SuffixSamplesCreator():
    ...
    def truncate_log(self, log, cut_off= '2016-11-29 10:41:28.348000'):
       # cutoff = pd.to_datetime(cut_off, format='%Y-%m-%dT%H:%M:%S.%f')

        # If sorting is unnecessary, you can remove this line
        chunk = log.sort_values(['end_timestamp'], ascending=True, kind='mergesort')
        midpoint = len(chunk) // 2
        cutoff = chunk.iloc[midpoint]['end_timestamp']
        cutoff = pd.to_datetime(cutoff, format='%Y-%m-%dT%H:%M:%S.%f')

        new_df_prefix = chunk.loc[chunk['end_timestamp'] < cutoff]
        new_df_suffix = chunk.loc[chunk['end_timestamp'] >= cutoff]

        return new_df_prefix, new_df_suffix
    ...
    def _sample_suffix(self,columns,parms):
        ...
        x_times_dict = dict()
        y_times_dict = dict()
        self.log = self.reformat_events(self.log, columns, parms['one_timestamp'])
        # n-gram definition
    ...
    def _sample_suffix_with_caseids(self, columns, parms):
        """
        Extraction of prefixes and expected suffixes from event log.
        Extraction of prefixes and suffixes
        Args:
        ...
        times = ['dur_norm'] if parms['one_timestamp'] else ['dur_norm', 'wait_norm']
        equi = {'ac_index': 'activities', 'rl_index': 'roles', 'caseid': 'caseid'}
        vec = {'prefixes': dict(),
               'next_evt': dict()}
        x_times_dict = dict()
        y_times_dict = dict()

        prefix_log , suffix_log = self.truncate_log(self.log)





        prefix_df = self.reformat_events(prefix_log, columns, parms['one_timestamp'])
        suffix_df = self.reformat_events(suffix_log, columns, parms['one_timestamp'])
        self.log = self.reformat_events(self.log, columns, parms['one_timestamp'])
        # n-gram definition
        columns.append('caseid')


        for i, _ in enumerate(prefix_df):
            for x in columns:
                serie, y_serie = list(), list()
                # for idx in range(1, len(prefix_df[i][x])):
                #     serie.append(self.log[i][x][:idx])
                #     y_serie.append(self.log[i][x][idx:])


                # Accessing dictionaries where 'caseid' is equal to some_value
                matching_suffix = [d for d in suffix_df if d.get('caseid') == prefix_df[i]['caseid'] ]
                try:
                    serie.append(prefix_df[i][x])
                    if x != 'caseid':
                        serie[0].pop()
                    ...
                    if not matching_suffix:
                        if x == 'ac_index':
                            ...
                            y_serie.append([self.ac_index[('end')]])
                        elif x == 'rl_index':
                            # serie.insert(0, self.rl_index[('start')])
                            y_serie.append([self.rl_index[('end')]])
                        else:
                            # serie.insert(0, 0)
                            y_serie.append([0])
                        ...
                        y_serie.append(matching_suffix[0][x])
                        if x != 'caseid':
                            y_serie[0].pop(0)
                            y_serie[0].pop()
                    ...
                    # Code to execute if IndexError occurs
                    print(f"Index is out of range for the list.")
