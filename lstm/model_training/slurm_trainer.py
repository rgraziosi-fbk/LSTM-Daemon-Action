...
class SlurmWorker():
    ...
    def __init__(self, argv):
        ...
        self.parms['one_timestamp'] = True  # Only one timestamp in the log
    ...
    def exec_pipeline(self):
        ...
        print("printitng shapes")
        print('train split size:', len(log_train), log_train.shape)
        print('valdn split size:', len(log_valdn), log_valdn.shape)
        # Vectorize input
    ...
    def _define_response(self, parms, loss, **kwargs) -> None:
        measurements = list()
        # Ensure 'dim' is a dictionary
        
        dim_dict = parms['dim'] if isinstance(parms['dim'], dict) else {}
